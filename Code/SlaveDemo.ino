...
BLEService TenseRoboSlave("0fcf5793-df75-475c-be63-62affc3fbbf9"); //UUID for turning a LED on
...
BLEByteCharacteristic OscStatus("aaf9", BLERead | BLENotify | BLEWrite); //UUID for Writing to A
...
void setup() {
  ...
  BLE.setLocalName("TensegrityRobotSlave"); //sets name for device
  BLE.setAdvertisedService(TenseRoboSlave);
  TenseRoboSlave.addCharacteristic(MotorPosition); //adds  to controlLed
  TenseRoboSlave.addCharacteristic(MotorControl); //adds  to controlLed
  TenseRoboSlave.addCharacteristic(OscStatus); //adds  to controlLed

  BLE.addService(TenseRoboSlave); //adds  to TensegrityRobot
  ...
  BLE.advertise(); //advertises TenseRoboSlave 
...
if(!setUp){
  ...
  if (central) { // central = device connected
    ...
    while(central.connected()) {
      MotorPosition.writeValue(Counter); //displays counter on an external variable
      ...
      if (MotorControl.value() != 0) {
        ...
        //Oscilate Slave once
        else if (MotorControl.value() == 53 && OscStatus.value() == 1) { //Hex Value 35
          if (Counter == stopValueA) {
            oscilationCounter = 1;
          }
          else if (Counter == 0){
